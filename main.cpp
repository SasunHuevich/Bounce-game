...
void collision(std::vector<sf::RectangleShape> walls){
}
...
int main()
{
    ...
    std::vector<sf::RectangleShape> wallsV;
    ...
    for (int i = 0; i < sizeof(lvl1map) / sizeof(lvl1map[0]); i++) {
        for (int j = 0; j < lvl1map[i].size(); j++) {
            ...
            else if (lvl1map[i][j] == 'V') {
                wall.setPosition(sf::Vector2f(j * 64.0, 256.0 + i * 64.0));
                wallsV.push_back(wall);
            }
            else if (lvl1map[i][j] == 'P') {
                shape.setPosition(sf::Vector2f(j * 64.0 + shapeWidth, 256.0 + i * 64.0 + shapeWidth));
            }
        }
    }
    ...
    float jumpSpeed = 0.00120;
    bool canJump = true;
    float timeKoef = 11000; // на моем компе в среднем за 11000 миллисекунд от начала тайма до мува
    ...
    while (window.isOpen())
    {
        ...
        for (auto &i : walls) {
            ...
            nextPos = shapeBounds;
            float nextPosX = (shape.getPosition().x) + movement.x * timeKoef;
            float nextPosY = (shape.getPosition().y) + movement.y * timeKoef;
            ...
            if (std::min({ d1_2, d2_3, d3_4, d4_1 }) < shapeWidth) {
               ...
               /*if (std::sqrt(std::pow((nextPosX - t1x), 2) + std::pow((nextPosY - t1y), 2)) < shapeWidth) {
                    //text5.setString("sssssssssssssssssssssss");
                    if (std::abs((nextPosX * t1x + nextPosY * t1y) / std::sqrt(std::pow(nextPosX, 2) + std::pow(nextPosY, 2))) < std::sqrt(2) / 2) {
                        text5.setString("1111111111111111111");
                    }
                    canJump = true;
                    velocityY = 0;
                    //movement.y = 0;
        ...
        for (auto& i : wallsV) {
            ...
            // Смещение проекций на радиус шара
            float deltaP1_2 = shapeWidth * 0.25 / std::sqrt(std::pow((t2x - t1x), 2) + std::pow((t2y - t1y), 2));
            float deltaP2_3 = shapeWidth * 0.25 / std::sqrt(std::pow((t3x - t2x), 2) + std::pow((t3y - t2y), 2));
            float deltaP3_4 = shapeWidth * 0.25 / std::sqrt(std::pow((t4x - t3x), 2) + std::pow((t4y - t3y), 2));
            float deltaP4_1 = shapeWidth * 0.25 / std::sqrt(std::pow((t1x - t4x), 2) + std::pow((t1y - t4y), 2));
            ...
            if (std::min({ d1_2, d2_3, d3_4, d4_1 }) < shapeWidth) {
                ...
                else if (d4_1 <= shapeWidth && p4_1 >= 0 - deltaP4_1 && p4_1 <= 1 + deltaP4_1) {
                    text5.setString("4_1 " + std::to_string(p4_1));
                    movement.x = 0;
                    shape.setPosition(sf::Vector2f(i.getPosition().x - shapeWidth, shape.getPosition().y));
                ...
                if (std::sqrt(std::pow((nextPosX - t1x), 2) + std::pow((nextPosY - t1y), 2)) <= shapeWidth) {
                    //text5.setString("sssssssssssssssssssssss");
                    if (shape.getPosition().x < t1x && shape.getPosition().y < t1y && movement.x > 0 && !shape.getPosition().y > t1y) {
                        text5.setString("xxxxxxxxxxxxxxxxxx");


                        /*sf::Vector2f Ax(shape.getPosition().x - t1x, t1y - t1y);
                        sf::Vector2f Ay(t1x - t1x, shape.getPosition().y - t1y);
                        sf::Vector2f Cc(shape.getPosition().x - t1x, shape.getPosition().y - t1y);

                        float lAx = std::sqrt(std::pow(Ax.x, 2) + std::pow(Ax.y, 2));
                        float lAy = std::sqrt(std::pow(Ay.x, 2) + std::pow(Ay.y, 2));
                        float lCc = std::sqrt(std::pow(Cc.x, 2) + std::pow(Cc.y, 2));

                        float XX = std::sqrt(std::pow(shapeWidth, 2) - std::pow(lAy, 2) - 2 * )*/

                        shape.setPosition(sf::Vector2f(t1x - shapeWidth, shape.getPosition().y));
                        movement.x = 0;
                    }
                    if (shape.getPosition().y < t1y && movement.y > 0) {
                        text5.setString("yyyyyyyyyyyyyyyyyyy");
                        canJump = true;
                        velocityY = 0;
                        //movement.y = 0;
                        clockJump.restart();
                        if (shape.getPosition().x < t1x  && movement.x > 0) {
                            movement.x = 0;
                        }
                        if (movement.y < 0.000001) {
                            movement.y = 0;
                        }
                    }
                    
                    /*sf::Vector2f d(shape.getPosition().x - t1x, shape.getPosition().y - t1y);
                    sf::Vector2f d_norm(d.x / std::sqrt(std::pow(d.x, 2) + (std::pow(d.y, 2))), d.y / std::sqrt(std::pow(d.x, 2) + (std::pow(d.y, 2))));

                    shape.setPosition(sf::Vector2f(shape.getPosition().x + shapeWidth * d_norm.x, shape.getPosition().y + shapeWidth * d_norm.y));
                    movement.x = 0;
                    movement.y = 0;*/
        ...
        /////////////////////////////////////////////////////////////////////////////////////////////////////////////
        //text5.setString(std::to_string(time));
        timeKoef = time;
        //text5.setString(std::to_string(movement.y));
        
        shape.move({ movement.x * time,movement.y * time }); // перемещение
        //shape.rotate(rotation);
        //text5.setString(std::to_string(shape.getPosition().x));
